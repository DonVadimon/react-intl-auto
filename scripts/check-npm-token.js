...
async function checkNpmToken(token) {
  ...
  try {
    ...
    const whoami = await makeRequest('https://registry.npmjs.org/-/whoami', token);
    
    if (whoami.error) {
      log('❌ Invalid or expired NPM token', 'red');
      log('Please create a new token at https://www.npmjs.com/settings/tokens', 'yellow');
      process.exit(1);
    }
    ...
    const packageInfo = await makeRequest('https://registry.npmjs.org/swc-plugin-react-intl-auto', token);
    
    if (packageInfo.error) {
      log('⚠️ Token cannot access package information', 'yellow');
      log('This might be normal if the package does not exist yet', 'yellow');
    } else {
    ...
    if (whoami.expires) {
      ...
      log(`📅 Token expires on: ${expiryDate.toISOString().split('T')[0]}`, 'blue');
    ...
    const testPackage = await makeRequest('https://registry.npmjs.org/@test-package-that-should-not-exist', token);
...
async function main() {
  ...
  log('\n📋 Next steps:', 'bold');
  log('1. If token is expired, create a new one at https://www.npmjs.com/settings/tokens', 'blue');
  log('2. Update the NPM_TOKEN secret in your GitHub repository settings', 'blue');
