...
function updateVersion(newVersion) {
  ...
  console.log(`✅ Updated version to ${newVersion} in both package.json and Cargo.toml`);
}
...
function pushToGit() {
  try {
    ...
    execSync(`git commit -m "chore: bump version to ${getCurrentVersion()}"`, { stdio: 'inherit' });
    execSync('git push origin main', { stdio: 'inherit' });
    execSync(`git push origin v${getCurrentVersion()}`, { stdio: 'inherit' });
...
function main() {
  ...
  console.log('2. Run: git push origin main && git push origin v' + newVersion);
