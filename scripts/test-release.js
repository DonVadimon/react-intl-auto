...
function checkRepository() {
  try {
    const repo = execSync('gh repo view --json nameWithOwner', { encoding: 'utf8' });
    const repoInfo = JSON.parse(repo);
    log(`✅ Repository: ${repoInfo.nameWithOwner}`, 'green');
    return true;
  } catch (error) {
    log('❌ Could not access repository. Make sure you have access.', 'red');
    return false;
  }
}
...
function checkRustSetup() {
  ...
  try {
    ...
    if (targets.includes('wasm32-wasip1')) {
      ...
      log('⚠️ wasm32-wasip1 target not installed. Run: rustup target add wasm32-wasip1', 'yellow');
    }
  } catch (error) {
...
async function main() {
  ...
  if (allChecksPassed) {
    log('🎉 All checks passed! Your release workflow should work correctly.', 'green');
    log('');
    log('Next steps:', 'blue');
    ...
    log('4. Delete the test tag: git tag -d v1.0.1-test && git push origin :v1.0.1-test', 'blue');
  } else {
    log('❌ Some checks failed. Please fix the issues above before releasing.', 'red');
    process.exit(1);
  }
}
