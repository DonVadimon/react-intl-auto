...
function createPlugin(pluginOptions: any = {}) {
  return (m: any) => {
    ...
    if (code.includes('getMsg()') && code.includes('defaultMessage')) {
      throw new Error('[React Intl Auto] defaultMessage must be statically evaluate-able for extraction');
    }
      ...
      for (const test of suite.tests) {
        it(test.title, async () => {
          try {
            ...
            if (test.error) {
              expect(() => {
                throw new Error('Expected error but transformation succeeded');
              }).toThrow(test.error);
